# Fallback to claude when a command is not found.

# Configure the prefix that marks commands for claude fallback.
...
command_not_found_handler() {
  ...
  if (( ${#effective_cmd[@]} == 0 )); then
    print -u2 "claude-cli: nothing to run after '${prefix_char}'."
    return 127
  fi
